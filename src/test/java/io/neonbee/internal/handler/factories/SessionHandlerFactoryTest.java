...
import static io.neonbee.internal.handler.factories.SessionHandlerFactory.METRIC_NAME;
import static io.neonbee.internal.handler.factories.SessionHandlerFactory.createSessionStore;
import static io.neonbee.internal.handler.factories.SessionHandlerFactory.getHostname;
...
class SessionHandlerFactoryTest {
    ...
    void testCreateHandlerLocal(Vertx vertx, VertxTestContext testContext) {
        try (MockedStatic<NeonBee> staticNeonBeeMock = mockStatic(NeonBee.class)) {
            ...
            when(neonBeeMock.getServerConfig()).thenReturn(serverConfig);
            when(neonBeeMock.getVertx()).thenReturn(vertx);
            when(neonBeeMock.getCompositeMeterRegistry()).thenReturn(new CompositeMeterRegistry());
            when(neonBeeMock.getNodeId()).thenReturn("test-node-id");
            staticNeonBeeMock.when(() -> NeonBee.get()).thenReturn(neonBeeMock);
    ...
    @Test
    void testSessionCountMetricRegistered(Vertx vertx, VertxTestContext testContext) {
        try (MockedStatic<NeonBee> staticNeonBeeMock = mockStatic(NeonBee.class)) {
            CompositeMeterRegistry registry = new CompositeMeterRegistry();
            NeonBee neonBeeMock = mock(NeonBee.class);
            ServerConfig serverConfig = new ServerConfig();
            serverConfig.setSessionHandling(SessionHandling.LOCAL);
            when(neonBeeMock.getServerConfig()).thenReturn(serverConfig);
            when(neonBeeMock.getVertx()).thenReturn(vertx);
            when(neonBeeMock.getCompositeMeterRegistry()).thenReturn(registry);
            when(neonBeeMock.getNodeId()).thenReturn("test-node-id");
            staticNeonBeeMock.when(() -> NeonBee.get()).thenReturn(neonBeeMock);
            ...
            new SessionHandlerFactory().createHandler()
                    .onComplete(testContext.succeeding(instance -> testContext.verify(() -> {
                        ...
                        assertThat(gauge.getId().getTag("node.id")).isEqualTo("test-node-id");
                        assertThat(gauge.value()).isAtLeast(0.0);
                        testContext.completeNow();
                    })));
        }
    }
    ...
    @Test
    void testSessionCountMetricNotRegisteredWhenNone(Vertx vertx, VertxTestContext testContext) {
        try (MockedStatic<NeonBee> staticNeonBeeMock = mockStatic(NeonBee.class)) {
            CompositeMeterRegistry registry = new CompositeMeterRegistry();
            NeonBee neonBeeMock = mock(NeonBee.class);
            ServerConfig serverConfig = new ServerConfig();
    ...
    @Test
    void testNoOpHandlerCallsNext() {
        RoutingContext routingContext = mock(RoutingContext.class);
        new SessionHandlerFactory.NoOpHandler().handle(routingContext);
        verify(routingContext).next();
    }

    @Test
    void testGetHostnameReturnsNonEmpty() {
