...
class ImmutableBufferTest {
    ...
    void testGetBuffer() {
        Buffer anyBuffer = Buffer.buffer("any");
        // violation to the immutable nature of the class, in order to safe the performance of having to copy the buffer
        // (which *would* be the right thing to do)
        assertThat(ImmutableBuffer.buffer(anyBuffer).getBuffer()).isSameInstanceAs(anyBuffer);
    }
