...
public final class ConfigHelper {
     ...
     *
     * What this method actually does it to extract all values of a configuration object, which are not part of a given
    ...
    public static JsonObject collectAdditionalConfig(JsonObject config, String... notAdditional) {
        ...
        return new JsonObject(config.stream().filter(valuePredicate(Objects::nonNull))
                .filter(keyPredicate(Predicate.not(notAdditionalSet::contains)))
                .collect(CollectionsHelper.identityMapCollector()));
    }
